...
class Exp_Main(Exp_Basic):
    ...
    def _select_criterion(self):
        if not self.args.freq_reg:
            criterion = nn.MSELoss()
        else:
            def time_freq_criterion(input, target):
                time_loss = torch.nn.functional.mse_loss(input, target, reduction='mean')

                input = torch.fft.fft(input, dim=-2, norm='forward')
                target = torch.fft.fft(target, dim=-2, norm='forward')
                freq_loss = torch.mean(torch.abs(input - target).pow(2))
                return time_loss + freq_loss
            criterion = time_freq_criterion
            print(f">>> Frequency Regression Loss is enabled.")
            print(f">>> Frequency Regression Loss is enabled.", file=sys.stderr)
        return criterion
