...
class Dataset_Custom(Dataset):
    ...
    def __read_data__(self):
        ...
        if os.path.exists(pickle_path):
            ...
            data = []
            for chunk in tqdm(chunks, total=total_rows // chunk_size + 1, desc="Reading data"):
                data.append(chunk)

            # 최종적으로 하나의 DataFrame으로 결합
            print("Concatenating", "Size: ", len(data))
            df_raw = pd.concat(data, ignore_index=True)
# chunk_size = 10000  # 청크 크기
# chunks = pd.read_csv(file_path, chunksize=chunk_size)

# data = []
# for chunk in tqdm(chunks, total=total_rows // chunk_size + 1):
#     data.append(chunk)
