...
class Exp_AliOFT(Exp_Main):
    def __init__(self, args):
        super(Exp_AliOFT, self).__init__(args)
    ...
    def _select_optimizer(self):
        if self.awl is None:
            return super(Exp_AliOFT, self)._select_optimizer()
    ...
    def train(self, setting):
        ...
        if 'finetune' in path:
            ...
            best_model_path = best_model_path.replace('-finetune', '')
        if not os.path.exists(best_model_path):
            print("No checkpoint found, training from scratch")
            raise FileNotFoundError("No checkpoint found")
        else:
            print("Loading checkpoint from {}".format(best_model_path))
